		...
		*/
		function __construct() //
		{
			...
			if( is_string( $isOver ) )
			{
				...
				{ header( 'Location: index.php?app_controller=info&type=' . $_GET[ 'type' ] . '&id=' . $isOver ); }
				else
				{ header( 'Location: info.php?type=' . $_GET[ 'type' ] . '&id=' . $isOver ); }
			}
			else if( $isOver )
			{
				$sys->drawRegistMaxCountOver( $gm, $loginUserType, $loginUserRank );
				print System::getFoot($gm,$loginUserType,$loginUserRank);
				System::flush();
				exit;
			}

			if( isset( $_GET[ 'mode' ] ) && 'registMaxCountOver' == $_GET[ 'mode' ] )
			{
				$sys->drawRegistMaxCountOver( $gm, $loginUserType, $loginUserRank );
				print System::getFoot($gm,$loginUserType,$loginUserRank);
				System::flush();
